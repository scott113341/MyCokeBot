...
export async function enterCode(code) {
  try {
    ...
    const pointsOrig = await nightmare.evaluate(getPoints);
    ...
    const pointsNow = await nightmare.evaluate(getPoints);
    const pointsNew = pointsNow - pointsOrig;
    const plural = pointsNew !== 1 ? 's' : '';

    if (pointsNew > 0) return `Thank you! Code ${code} added ${pointsNew} point${plural} for a total of ${pointsNow}.`;
